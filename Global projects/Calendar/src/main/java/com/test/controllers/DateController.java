...
public class DateController {
	...
	public String findCalendarOnMonth(Model model, @RequestParam(value = "y", defaultValue = "2021") int year,
		...
		try {
			List<DateEntity> previous = dateService.findLastDaysInPreviousMonth(month, year);
			List<DateEntity> current = dateService.findMonth(month, year);
			List<DateEntity> nextDays = dateService.findFirstDaysInPreviousMonth(month, year);
			String stringMonth = CalendarImpl.getStringMonth(month);
	...
	public String findCalendar(Model model, @RequestParam(value = "y", defaultValue = "2021") int year,
		...
		try {
			DateEntity d = dateService.findDay(day, month, year);
